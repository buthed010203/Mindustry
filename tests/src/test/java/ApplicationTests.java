    ...
    DynamicTest[] testSectorValidity(){
        ...
        for(SectorPreset zone : content.sectors()){
            ...
            out.add(dynamicTest(zone.name, () -> {
                ...
                assertTrue(spawner.countSpawns() > 0 || (state.rules.attackMode && state.teams.get(Team.crux).hasCore()), "Sector \"" + zone.name + "\" has no enemy spawn points: " + spawner.countSpawns());
            }));
        }
